...
import unittest
import time
from common import HTMLTestRunner_lu
...
def add_case(caseName="testcase", rule="test*.py"):
    ...
    # 如果不存在这个testcase文件夹，就自动创建一个
    if not os.path.exists(case_path):os.mkdir(case_path)
    print("testcase path:%s"%case_path)
    # 定义discover方法的参数
...
def run_case(all_case, reportName="report"):
    ...
    report_abspath = report_path+'\\'+now+'report.html'
    print("report path:%s"%report_abspath)
    fp = open(report_abspath, "wb")
    runner = HTMLTestRunner_lu.HTMLTestRunner(stream=fp,
...
if __name__ == "__main__":
    all_case = add_case(caseName="testcase", rule="test*.py")   # 1 加载用例
